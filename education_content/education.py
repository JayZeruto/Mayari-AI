# Education module for Mayari.
# This module handles the detection of educational queries, malicious requests,
# and the generation of safe learning responses.
import re

class EducationModule:
    def __init__(self):
        # Pattern to identify questions that are likely educational in intent.
        ...
        # Patterns that match requests for harmful or disallowed content.
        # These are used to block dangerous prompts while still allowing safe teaching.
        ...
        # Default response when a query appears malicious or unsafe.
        self.safe_refusal = (
        ...
        # Placeholder for future inline education content items.
        self.content_items = []

    def normalize(self, user_input):
        # Normalize the input by stripping whitespace and lowercasing it.
        # This makes pattern matching consistent and case-insensitive.
        return re.sub(r"\s+", " ", user_input.strip().lower())

    def is_malicious_request(self, user_input):
        # Check whether the normalized user query matches any malicious patterns.
    ...
    def is_educational_query(self, user_input):
        # Determine if the user query should be treated as an educational question.
        normalized = self.normalize(user_input)
        if self.is_malicious_request(normalized):
            # If it looks unsafe, do not treat it as educational.
            return False

        if self.student_intent.search(normalized):
            # If the query contains words like "explain" or "teach", it's educational.
            return True

        # If there is matching content available, it is also educational.
    ...
    def find_content(self, normalized_input):
        # Attempt to find matching educational content based on the normalized input.
        # This method currently searches an inline content list if it is populated.
        ...
        # If no exact keyword match is found, fall back to returning any content
        # that matches the detected education level.
